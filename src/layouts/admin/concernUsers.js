...
function ConcernUsers() {
  ...
  useEffect(() => {
    axios.get("/api/all-fraud-users-list").then((res) => {
      if (res.data.status === 200) {
        setData(res.data.concernProfile);
        setFilteredData(res.data.concernProfile);
      }
    ...
    dataList = paginatedData.map((row) => {
      return (
        <tr key={row.id}>
          <td>{row.id}</td>
          <td>{row.user_id}</td>
          <td style={{ padding: "14px 4px" }}>{row.blocker_user_id}</td>

          <td>{row.reason}</td>

          <td>{row.status == "1" ? "Blocked" : "Reported"}</td>

          <td>{dataFormat(row.updated_at)}</td>
          ...
          <td>
            ...
            <Link
              ...
              onClick={(e) => blockRow(e, row.blocker_user_id)}
            >
            ...
            <button
              ...
              onClick={(e) => deleteRow(e, row.blocker_user_id)}
              style={{ borderRadius: "50%" }}
            >
  ...
  return (
    <>
      <div className="d-sm-flex align-items-center justify-content-between m-4">
        <h1 className="h3 mb-0 text-gray-800">All Reported User Details</h1>
